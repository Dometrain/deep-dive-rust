...
/// 2.2 #3 -- Lifetime Elision Rules.
pub mod lifetime_elision {
    /// Written in full, this signature would be
    /// `fn first_word<'a>(s: &'a str) -> &'a str`. Elision rule 2 ("one
    /// input lifetime -> every output gets it") lets you drop the `<'a>`
    /// entirely; `crate::models::task::TaskTitle::as_str` is the same rule
    /// at work in the real codebase.
...
/// 2.2 #4 -- Variance.
///
/// No real-app tie-in for this one, by design -- you will not consciously
/// invoke variance in this codebase. It matters because it explains *why*
/// certain borrow-checker errors show up (or don't) once lifetimes and
/// generics combine, not because you'll write code that names it.
pub mod variance {
    // -- Covariance: `&'a T` is covariant over `'a` -------------------------
    // A longer-lived reference is always usable where a shorter-lived one
    // is expected -- a `&'static str` satisfies any `&'a str` bound. That's
    // covariance, and it's why the call in the test below compiles without
    // either input lifetime being written out.
    ...
    mod covariance_tests {
        ...
        #[test]
        fn a_static_str_satisfies_a_shorter_lifetime_bound() {
            let long_lived: &'static str = "static and long-lived";
            let short_lived = String::from("short");
            // `long_lived` outlives `short_lived` by a lot, but `shortest`
            // only asks for *some* shared lifetime `'a` -- the compiler
            // unifies both to the shorter one (tied to `short_lived`)
            // automatically, because `&'static str` is a subtype of
            // `&'a str` for every `'a`.
            assert_eq!(shortest(long_lived, &short_lived), "short");
        }
    }

    // -- Contravariance: rare enough in safe Rust that it's more honest to
    // explain than to fake-demonstrate ---------------------------------------
    //
    // The lesson doc's own code sample for this (a plain `Fn(&i32) -> i32`
    // passed into a higher-order function) doesn't actually exercise
    // contravariance -- no lifetime subtyping happens in it at all. A real
    // demonstration needs two `fn` *pointer types* compared against each
    // other, and the direction only matters at the type-checker level, not
    // as behavior you can assert on in a test:
    //
    //   fn accepts_only_static(f: fn(&'static str)) { /* ... */ }
    //   fn accepts_any_lifetime<'a>(f: fn(&'a str)) { /* ... */ }
    //
    //   // A function pointer that can handle *any* lifetime is usable
    //   // wherever one that only handles `'static` was expected -- but not
    //   // the other way around. The subtyping direction is the reverse of
    //   // the reference case above; that reversal is what "contra" means.

    // -- Invariance: `Cell<T>`/`RefCell<T>`/`Mutex<T>` are invariant over T -
    //
    // These types let you write a new `T` in through a shared reference --
    // that's interior mutability, lesson 2.1. If `Cell<T>` were covariant
    // like `&T`, this would compile:
    //
    //   fn shorten<'a>(
    //       cell: std::cell::Cell<&'static str>,
    //   ) -> std::cell::Cell<&'a str> {
    //       cell // pretend this coercion were allowed
    //   }
    //
    // ...and a caller could then `.set()` a short-lived `&'a str` into a
    // `Cell` whose original owner still believes it only ever holds
    // `&'static` strings -- a dangling reference the moment `'a` ends.
    // `Cell<T>` (and `RefCell`, `Mutex`, `&mut T`) are invariant over `T`
    // specifically to rule this out at compile time, not by convention.
}
